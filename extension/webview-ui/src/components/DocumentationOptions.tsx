...
import { StyledSelect, StyledLabel } from './common/Select';
...
import { fetchAnalysis, fetchAnnotation, fetchDefinition, fetchRevision } from "../api/services";
...
const documentationOptionsMetaData = [
  {
    ...
    description: 'Create framework specific definitions for your functions and classes \
    for easier manual documentation integrations and quick standardization!',
    onClick: (framework: string | undefined = undefined) => vscode.postMessage({
      command: 'define', text: framework
    ...
    description: 'Generate line by line comment to explain complex functions with relative ease. \
    ...
    description: 'Streamline your variable names with clear and concise names following a casing scheme of your choice.',
    onClick: (casing_scheme: string | undefined = undefined) => vscode.postMessage({
      command: 'revise', text: casing_scheme
    ...
    description: 'Understand your code efficiency and optimize code execution times by analyzing runtime complexity.',
...
const AccordionSummary = styled((props: AccordionSummaryProps) => (
  <MuiAccordionSummary
    expandIcon={<ArrowForwardIosSharpIcon sx={{ fontSize: '0.9rem', color: '#feefb7' }} />}
    {...props}
  />
))(({ theme }) => ({
  backgroundColor: 'transparent',
...
const AccordionDetails = styled(MuiAccordionDetails)(({ theme }) => ({
  ...
  backgroundColor: 'transparent',
  borderTop: '2px solid #feefb7',
}));


const DocumentationOptionAlternativeSection = ({
  section: { sectionTitle, buttonTitle, hasGroupedOptions, options, defaultOption },
  onChangeSelectedOption, sectionSelectedOption }: any) => {
                  ...
                  Object.keys(options).map((groupName) => (
                    <optgroup key={groupName} label={groupName}>{
                      (options as any)[groupName].map((groupElement: any) => (
                        <option key={`${groupName}-${groupElement}`} value={groupElement}>
                          {groupElement}
                        </option>
                      ))
                    }</optgroup>
...
const DocumentationAccordion = ({ loading }: { loading: { [option_id: string]: boolean } }) => {
      ...
      setExpanded(() => {
        if (typeof expanded === 'boolean') {
          ...
          if (expanded.includes(panel)) {
            return newExpanded
              ? expanded : expanded.filter((id) => id != panel)
          }
          else { return newExpanded ? expanded.concat(panel) : expanded }
        }
      });
    };
  ...
  return (
    <div>
      {
        documentationOptionsMetaData.map(({ name, buttonName, description, id, secondSection, onClick }) => (
          <Accordion key={id} expanded={typeof expanded === 'boolean'
            ? expanded : expanded.includes(id)} onChange={handleAccordionStateChange(id)}>
            <AccordionSummary aria-controls="documentation-accordian-content" id="documentation-accordian-header">
            ...
            <AccordionDetails>
              ...
              <div style={responsiveDivStyles}>
                {secondSection && (<DocumentationOptionAlternativeSection {...{
                ...
                <Box {...{
                  ...
                  <Box {...{
                    ...
                    <StyledLoadingButton {...{
                      loading: loading[id],
                      onClick: () => onClick(selectedOptions[id] || undefined),
                      variant: 'outlined',
                    }} fullWidth>
                      {buttonName}
                    </StyledLoadingButton>
...
const DocOptions = () => {
  ...
  const loadingWrapper = async (fn: Function, args: any[], option_id: string, error_msg: string) => {
    try {
  ...
  const getDefinitionOutput = async (inputCode: string, option_id: string, language?: string, framework?: string) => {
    return await loadingWrapper(fetchDefinition, [inputCode, language, framework], option_id,
      'Failed to create definitions...\n Please try again!')
  };

  const getRevisionOutput = async (inputCode: string, option_id: string, language?: string, caseType?: string) => {
    return await loadingWrapper(fetchRevision, [inputCode, language, caseType], option_id,
      'Failed to create variable revisions...\n Please try again!')
  };

  const getAnnotationOutput = async (inputCode: string, option_id: string, language?: string) => {
    return await loadingWrapper(fetchAnnotation, [inputCode, language], option_id,
      'Failed to annotate code snippet...\n Please try again!')
  };

  const getAnalysisOutput = async (inputCode: string, option_id: string, language?: string) => {
    return await loadingWrapper(fetchAnalysis, [inputCode, language], option_id,
      'Failed to analyze code snippet...\n Please try again!')
  };

  const performOperations = async ({ command, language,
    response: { content, range }, additional_context }: any) => {
    let result: any;
    switch (command) {
      case 'define':
      ...
      case 'analyze':
        ...
        if (result.status && result.content.status) {
          ...
          vscode.postMessage({ command: 'error', text: !result.status ? result.error_msg : result.content.error_msg })
        }
        break;

      case 'revise':
        result = await getRevisionOutput(content, command, language, additional_context);
        if (result.status && result.content.status) {
          vscode.postMessage({
            command: 'update',
            text: JSON.stringify({ range, content: result.content.content })
  ...
  return (
    <main style={{
      ...
      <br />
      <Box sx={{
        flexDirection: "column", borderRadius: '15px', marginTop: '2px',
        width: '90%', alignItems: 'center', justifyContent: 'center'
      }}>
        <Box sx={{
          width: '100%', color: '#feefb7',
          alignItems: 'center', justifyContent: 'center'
        }}>
          <Typography>
            <h3>External Documentation Generation</h3>
          </Typography>

          <Typography sx={{ textAlign: 'justify' }}>
            We provide a means for you to create holistic text based documents that explain your files and modules.
            This is the quickest way to accurately summarize your commits in order for your managers to get up to speed with your contributions.
            You can generate your text documents in PDF or Markdown Format.
          </Typography>
        </Box>
